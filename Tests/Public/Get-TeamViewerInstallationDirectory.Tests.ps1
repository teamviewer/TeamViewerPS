...
Describe 'Get-TeamViewerInstallationDirectory' {
    BeforeAll {
        ...
        $testItem | Add-Member `
            -MemberType ScriptMethod -Name GetValue -Value { param($obj) Get-TestItemValue @PSBoundParameters }
    ...
    It 'Should check the registry path and return the installation directory' {
        ...
        Should -Invoke Test-Path -Scope It -Times 1 -ParameterFilter {
            $Path -eq 'testRegistry' -and $PathType -eq 'Container'
        }
    ...
    It 'Should return null if registry path does not exist' {
        Mock Test-Path -ParameterFilter { $Path -eq 'testRegistry' -and $PathType -eq 'Container' } { $false }
