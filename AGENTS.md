...
- Output types: All public cmdlets must declare `[OutputType()]` when their pipeline output is clear and stable. Verify the runtime output type and document the same type in the corresponding help file's `OUTPUTS` section. Use CLR types for primitive outputs and `[void]` for commands that intentionally emit no pipeline output. For converter-backed `PSCustomObject` outputs, use the existing custom type name as a string (for example, `[OutputType('TeamViewerPS.User')]`). Declare multiple types when output is intentionally conditional. Do not guess a type for ambiguous or raw REST responses; standardize the runtime output first. Keep the help file's `OUTPUTS` section consistent with the attribute.
